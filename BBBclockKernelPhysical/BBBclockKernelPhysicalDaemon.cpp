...
CKPD::CKPD(){// Redeclaration of constructor GPIO when no argument is specified
	...
	*/
	...
	std::chrono::nanoseconds duration_back(static_cast<unsigned long long int>(ceil(static_cast<long double>(std::chrono::duration_cast<std::chrono::nanoseconds>(TimePointClockCurrentInitial.time_since_epoch()).count())/static_cast<long double>(this->TimeAdjPeriod))*static_cast<long double>(this->TimeAdjPeriod)));
