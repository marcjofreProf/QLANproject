 ...
 * 2024
 */
#include"BBBclockKernelPhysicalDaemonSignalDriver.h"
#include <iostream>
...
void* exploringBBBCKPDSD::CKPDSD::ddrMem = nullptr; // Define and initialize ddrMem
void* exploringBBBCKPDSD::CKPDSD::sharedMem = nullptr; // Define and initialize
void* exploringBBBCKPDSD::CKPDSD::pru0dataMem = nullptr; // Define and initialize 
void* exploringBBBCKPDSD::CKPDSD::pru1dataMem = nullptr; // Define and initialize
void* exploringBBBCKPDSD::CKPDSD::pru_int = nullptr;// Define and initialize
unsigned int* exploringBBBCKPDSD::CKPDSD::sharedMem_int = nullptr;// Define and initialize
unsigned int* exploringBBBCKPDSD::CKPDSD::pru0dataMem_int = nullptr;// Define and initialize
unsigned int* exploringBBBCKPDSD::CKPDSD::pru1dataMem_int = nullptr;// Define and initialize
int exploringBBBCKPDSD::CKPDSD::mem_fd = -1;// Define and initialize 
...
//}
///////////////////////////////////////////////////////
CKPDSD::CKPDSD(){// Redeclaration of constructor GPIO when no argument is specified
	...
	//pru0dataMem_int[1]=(unsigned int)0; // set to zero means no command. PRU0 idle
	  ...
	  pru0dataMem_int[1]=static_cast<unsigned int>(0); //no command
...
// Send and receive information for the next round
...
int CKPDSD::RelativeNanoSleepWait(unsigned int TimeNanoSecondsSleep){
struct timespec ts;
...
double CKPDSD::DoubleMedianFilterSubArray(double* ArrayHolderAux){
...
double CKPDSD::DoubleMeanFilterSubArray(double* ArrayHolderAux,unsigned long long int FilterFactor){
if (FilterFactor<=1){
	return ArrayHolderAux[0];
}
else{
...
int CKPDSD::DoubleBubbleSort(double* arr) {
...
unsigned long long int CKPDSD::ULLIMedianFilterSubArray(unsigned long long int* ArrayHolderAux){
if (this->MedianFilterFactor<=1){
	return ArrayHolderAux[0];
}
else{
...
unsigned long long int CKPDSD::ULLIMeanFilterSubArray(unsigned long long int* ArrayHolderAux){
...
// Function to implement Bubble Sort
int CKPDSD::ULLIBubbleSort(unsigned long long int* arr) {
    for (int i = 0; i < this->MedianFilterFactor-1; i++) {
        for (int j = 0; j < this->MedianFilterFactor-i-1; j++) {
            if (arr[j] > arr[j+1]) {
                // Swap arr[j] and arr[j+1]
...
int CKPDSD::IMedianFilterSubArray(int* ArrayHolderAux){
if (this->MedianFilterFactor<=1){
	return ArrayHolderAux[0];
}
else{
...
int CKPDSD::IMeanFilterSubArray(int* ArrayHolderAux){
if (this->MeanFilterFactor<=1){
	return ArrayHolderAux[0];
}
else{
...
// Function to implement Bubble Sort
int CKPDSD::IBubbleSort(int* arr) {
    for (int i = 0; i < this->MedianFilterFactor-1; i++) {
        for (int j = 0; j < this->MedianFilterFactor-i-1; j++) {
            if (arr[j] > arr[j+1]) {
                // Swap arr[j] and arr[j+1]
...
using namespace exploringBBBCKPDSD;
