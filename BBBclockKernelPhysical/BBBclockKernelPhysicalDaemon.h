...
private:// Variables
	...
	double PIDconstant=1.2; // Since clock tend to either go faster or slower with respect the reference clock, this value can be larger than 1 without having unstability. The larger than 1 the more aggressive correction. Below 1.0 is not aggressively enough to correct fully, eventhought it will try. This value times the maxium value set in MaxTimePointClockCurrentAdjError, has not ot exceed the period wanted. It has to be larger than the jitter of the hardware clocks
	// Maximum values
