...
#define ClockCyclePeriodAdjustment		32768 // Very important parameter. The larger the better, since the interrupts time jitter do not paly a role, as long as the PRU counter does not overexceed (the turn down is that ht eupdate time is larger)
...
public: //Variables
	...
	unsigned int NumClocksHalfPeriodPRUclock=static_cast<unsigned int>(0.25*FactorTimerAdj*(static_cast<double>(ClockPeriodNanoseconds))/(static_cast<double>(PRUclockStepPeriodNanoseconds)));// set the number of clocks that defines the half period of the clock. For 32Khz, with a PRU clock of 5ns is 6250
