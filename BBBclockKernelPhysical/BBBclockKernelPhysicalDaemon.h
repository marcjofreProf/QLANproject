...
#define PRUclockStepPeriodNanoseconds		4.999743 // Very critical parameter experimentally assessed. PRU clock cycle time in nanoseconds. Specs says 5ns, but maybe more realistic is the 24 MHz clock is a bit higher and then multiplied by 8
...
private:// Variables
	...
	double TimePointClockCurrentAdjFilErrorAppliedOld=0;
	...
	TimePointWatch TimePointClockCurrentFinalExtra=std::chrono::time_point<ClockWatch>(); // Initial updated value of the clock (updated in each iterati
	...
	double PIDconstant=0.25; // The larger than 1 the more aggressive correction. Below 1.0 is not aggressively enough to correct fully, eventhought it will try. This value times the maxium value set in MaxTimePointClockCurrentAdjError, has not ot exceed the period wanted. It has to be larger than the jitter of the hardware clocks
