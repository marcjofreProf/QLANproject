...
public: //Variables
	...
	int FilterMode=1; // 0: averaging; 1: median; 2: mean window. The erro jumps between two values, then maybe it is better to use mea window
...
private:// Variables
	...
	double PIDconstant=0.5; // The larger than 1 the more aggressive correction (and more unstable). Below 1.0 is not aggressively enough to correct fully, eventhought it will try. This value times the maxium value set in MaxTimePointClockCurrentAdjError, has not ot exceed the period wanted. It has to be larger than the jitter of the hardware clocks
	// Maximum values
	...
	int MaxTimePointClockCurrentAdjError=NumClocksHalfPeriodPRUclock/10;
	int MinTimePointClockCurrentAdjError=-NumClocksHalfPeriodPRUclock/10;
	int FilterModeHolder=2;
