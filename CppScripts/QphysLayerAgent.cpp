	...
	if (RawTotalCurrentNumRecordsQuadCh[SpecificQuadChDet]>0){
		if (UseAllTagsForEstimation){
			...
			for (unsigned int i=0;i<RawTotalCurrentNumRecordsQuadCh[SpecificQuadChDet];i++){
				...
				SynchFirstTagsArrayAux[i]=(static_cast<long long int>(TimeTaggs[SpecificQuadChDet][i])-SynchTimeTaggRef[iCenterMass][iNumRunsPerCenterMass]+ChOffsetCorrection*LLIHistPeriodicityAux)%(LLIHistPeriodicityAux);
			...
			SynchFirstTagsArray[iCenterMass][iNumRunsPerCenterMass]=(static_cast<long long int>(TimeTaggs[SpecificQuadChDet][0])-SynchTimeTaggRef[iCenterMass][iNumRunsPerCenterMass]+ChOffsetCorrection*LLIHistPeriodicityAux)%(LLIHistPeriodicityAux);
	...
	if (RawTotalCurrentNumRecordsQuadCh[SpecificQuadChDet]>0){	
		if (UseAllTagsForEstimation){
			...
			for (unsigned int i=0;i<RawTotalCurrentNumRecordsQuadCh[SpecificQuadChDet];i++){
				...
				SynchFirstTagsArrayAux[i]=(static_cast<long long int>(TimeTaggs[SpecificQuadChDet][i])-SynchTimeTaggRef[iCenterMass][iNumRunsPerCenterMass]+ChOffsetCorrection*LLIHistPeriodicityAux)%(LLIMultFactorEffSynchPeriod*LLIHistPeriodicityAux);//(LLIHistPeriodicityHalfAux+static_cast<long long int>(TimeTaggs[i]))%LLIHistPeriodicityAux-LLIHistPeriodicityHalfAux;//static_cast<long long int>(TimeTaggs[i])%LLIHistPeriodicityAux;
				if (i%150==0){// To be commented when not debugging
			...
			// Single value
			ChOffsetCorrection=static_cast<long long int>(BitPositionChannelTags(ChannelTags[SpecificQuadChDet][0])%4);// Maps the offset correction for the different channels to detect a states
			SynchFirstTagsArrayOffsetCalc[iNumRunsPerCenterMass]=(static_cast<long long int>(TimeTaggs[SpecificQuadChDet][0])-SynchTimeTaggRef[iCenterMass][iNumRunsPerCenterMass]+ChOffsetCorrection*LLIHistPeriodicityAux)%(LLIMultFactorEffSynchPeriod*LLIHistPeriodicityAux);//(LLIHistPeriodicityHalfAux+static_cast<long long int>(TimeTaggs[0]))%LLIHistPeriodicityAux-LLIHistPeriodicityHalfAux;//static_cast<long long int>(TimeTaggs[0])%LLIHistPeriodicityAux; // Considering only the first timetagg. Might not be very resilence with noise
			cout << "QPLA::Using only first timetag for network synch computations!...to be deactivated" << endl;
		}
		//cout << "QPLA::SynchFirstTagsArray[iCenterMass][iNumRunsPerCenterMass]: " << SynchFirstTagsArray[iCenterMass][iNumRunsPerCenterMass] << endl;
	}	
