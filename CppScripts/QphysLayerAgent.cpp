				...
				if (RawTotalCurrentNumRecordsQuadCh[iQuadChIter]>0){
					...
					for (int i=0;i<CurrentNumIdentifiedEmitReceiveIP;i++){
						if (QuadChannelParamsLink[i]==iQuadChIter){
							if (CurrentSpecificLinkAux<0){CurrentSpecificLinkAux=i;}// Take the first identified, which is the one that matters most
						}
					}
				  ...
				  if (NonInitialReferencePointSmallOffsetDriftPerLink[iQuadChIter][CurrentSpecificLinkMultiple]==false){			
					  ...
					  // Done afterwards NonInitialReferencePointSmallOffsetDriftPerLink[iQuadChIter][CurrentSpecificLinkMultiple]=true;// Update value, so that it is not run again
					}
				  // First compute the relative new time offset from last iteration
					...
					if (UseAllTagsForEstimation){						
					  ...
					  //cout << "QPLA::SmallDriftContinuousCorrection SmallOffsetDriftAux: " << SmallOffsetDriftAux << endl;
						...
						if (LLIMultFactorEffSynchPeriod==4){// When using histogram analysis
							ChOffsetCorrection=static_cast<long long int>(BitPositionChannelTags(ChannelTags[iQuadChIter][0]));// Maps the offset correction for the different channels to detect a specific state
...
if (SimulateNumStoredQubitsNodeAux>1){
	...
	CoincidenceWindowPRU=static_cast<long long int>(1.35*HistPeriodicityAux); // Adjusted empirically, or something else needs to be further adjusted.
	...
	if (RawTotalCurrentNumRecordsQuadCh[SpecificQuadChDet]>0){
		if (UseAllTagsForEstimation){
			...
			for (unsigned int i=0;i<RawTotalCurrentNumRecordsQuadCh[SpecificQuadChDet];i++){
				ChOffsetCorrection=static_cast<long long int>(BitPositionChannelTags(ChannelTags[SpecificQuadChDet][i]));// Maps the offset correction for the different channels to detect a specific state
...
if (iCenterMass==0){// Here the modulo is dependent on the effective period		
	if (RawTotalCurrentNumRecordsQuadCh[SpecificQuadChDet]>0){	
		if (UseAllTagsForEstimation){
			...
			for (unsigned int i=0;i<4;i++){CheckChOffsetCorrectionIter[i]=0;}// Reset values
			for (unsigned int i=0;i<RawTotalCurrentNumRecordsQuadCh[SpecificQuadChDet];i++){
				ChOffsetCorrection=static_cast<long long int>(BitPositionChannelTags(ChannelTags[SpecificQuadChDet][i]));// Maps the offset correction for the different channels to detect a states
				//cout << "ChOffsetCorrection: " << ChOffsetCorrection << endl;
...
if (iCenterMass==(NumCalcCenterMass-1) and iNumRunsPerCenterMass==(NumRunsPerCenterMass-1)){// Achieved number measurements to compute values
	...
	for (int i=0;i<NumRunsPerCenterMass;i++){
		// Offset wrapping
		SynchCalcValuesArrayAux[i]=static_cast<double>(SynchFirstTagsArrayOffsetCalc[i]);//static_cast<double>((LLIMultFactorEffSynchPeriod*LLIHistPeriodicityHalfAux+SynchFirstTagsArrayOffsetCalc[i])%(LLIMultFactorEffSynchPeriod*LLIHistPeriodicityAux)-(LLIMultFactorEffSynchPeriod*LLIHistPeriodicityHalfAux));
