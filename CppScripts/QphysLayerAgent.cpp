...
cout << "When not using hist analys,it can be changed back to 2048 NUMRECORDS in PRUassTaggDetScript.p, GPIO.h and top of QphysLayerAgent.cpp" << endl;
...
TimeTaggsDetAnalytics[5]=TimeTaggsDetAnalytics[5]+(1.0/((double)SimulateNumStoredQubitsNodeAux-1.0))*(((double)((HistPeriodicityAux/2+TimeTaggs[i+1]-TimeTaggs[i])%HistPeriodicityAux)+1.0)-(double)(HistPeriodicityAux/2));
}

for (int i=0;i<(SimulateNumStoredQubitsNodeAux-1);i++){
TimeTaggsDetAnalytics[6]=TimeTaggsDetAnalytics[6]+(1.0/((double)SimulateNumStoredQubitsNodeAux-1.0))*pow(((double)((HistPeriodicityAux/2+TimeTaggs[i+1]-TimeTaggs[i])%HistPeriodicityAux)+1.0)-(double)(HistPeriodicityAux/2)-TimeTaggsDetAnalytics[5],2.0);
}
TimeTaggsDetAnalytics[6]=sqrt(TimeTaggsDetAnalytics[6]);
