					...
					if (UseAllTagsForEstimation){
						...
						for (unsigned int i=0;i<4;i++){
							for (unsigned int j=0;j<4;j++){
								if (i!=j and abs(CheckChOffsetCorrection[i]-CheckChOffsetCorrection[j])>(3*LLIHistPeriodicityAux/4) and CheckChOffsetCorrectionIter[i]>0 and CheckChOffsetCorrectionIter[j]>0){
...
if (iCenterMass==0){// Here the modulo is dependent n the effective period		
	if (RawTotalCurrentNumRecordsQuadCh[SpecificQuadChDet]>0){	
		if (UseAllTagsForEstimation){
			...
			for (unsigned int i=0;i<4;i++){
				for (unsigned int j=0;j<4;j++){
					if (i!=j and abs(CheckChOffsetCorrection[i]-CheckChOffsetCorrection[j])>(3*LLIHistPeriodicityAux/4) and CheckChOffsetCorrectionIter[i]>0 and CheckChOffsetCorrectionIter[j]>0){ // If tehre are counts in each pin compared
						boolCheckChOffsetCorrectionflag=true;
