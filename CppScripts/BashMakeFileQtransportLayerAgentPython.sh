# The Python folder in /usr/include/pythonX.XX (and specifically the file Python.h) has to exist. Actually, it is good to copy the compressed folder python3.10 to the location /usr/include/ as "sudo tar -xvzf python3_10.tar.gz -C /usr/include/python3.10 (first create python3.10 directory "sudo mkdir python3.10")
