...
int isValidWhileLoopCount = 20; // Number of tries If it needs more than one trial is because the sockets are not working correctly. It is best to check for open sockets and kill the processes taking hold of them
...
while(isValidWhileLoopCount>0){
	...
	this->release();
	this->RelativeNanoSleepWait((unsigned long long int)(80*(WaitTimeAfterMainWhileLoop*(1.0+(float)rand()/(float)RAND_MAX))));//usleep((int)(500*WaitTimeAfterMainWhileLoop*(1.0+(float)rand()/(float)RAND_MAX)));// Give some time to have the chance to receive the response
	this->acquire();
...
this->release();
	this->RelativeNanoSleepWait((unsigned long long int)(100*(WaitTimeAfterMainWhileLoop*(1.0+(float)rand()/(float)RAND_MAX))));//usleep((int)(500*WaitTimeAfterMainWhileLoop*(1.0+(float)rand()/(float)RAND_MAX)));// Give some time to have the chance to receive the response
	this->acquire();
	if (this->InfoSimulateNumStoredQubitsNodeFlag==true){
		//cout << "We received info for SimulateRetrieveNumStoredQubitsNode" << endl;
...
int QTLAH::WaitUntilActiveActionFreePreLock(char* ParamsCharArrayArg, int nChararray){	
	try{
		...
		AchievedAttentionParticularHosts=false;
		int MaxNumPassesCheckBlockAux=50; // Protection to not block permanently
		...
		while((AchievedAttentionParticularHosts==false or FirstPassAux==true) and AchievedAttentionParticularHosts==false and NumPassesCheckBlockAux<MaxNumPassesCheckBlockAux){
			...
			while ((BusyAttachedNode==true or HostsActiveActionsFree[0]==false or GPIOnodeHardwareSynched==false or GPIOnodeNetworkSynched==false) and AchievedAttentionParticularHosts==false and NumPassesCheckBlockAux<MaxNumPassesCheckBlockAux){// Wait here// No other thread checking this info
				...
				//cout << "GPIOnodeHardwareSynched: " << GPIOnodeHardwareSynched << endl;
				//cout << "GPIOnodeNetworkSynched: " << GPIOnodeNetworkSynched << endl;
				if (FirstPassAux==false and NumPassesCheckBlockAux>(MaxNumPassesCheckBlockAux)){cout << "Host " << this->IPaddressesSockets[2] << " waiting network & hardware synchronization or availability of other hosts to proceed with the request!" << endl;}
