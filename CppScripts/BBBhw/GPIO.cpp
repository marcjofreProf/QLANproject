...
GPIO::GPIO(){// Redeclaration of constructor GPIO when no argument is specified
	...
	pru1dataMem_int[7]=static_cast<unsigned int>(SigONPeriod);
	...
	*/
	...
	sleep(90); // Give some time to load programs in PRUs and the synch protocols to initiate and lock after prioritazion and adjtimex. Very important, otherwise bad values might be retrieved
		...
		if (this->ManualSemaphoreExtra==false){// It was possible to execute when needed			
			...
			if (this->ResetPeriodicallyTimerPRU1){
				...
				if ((iIterPRUcurrentTimerValSynch%static_cast<unsigned long long int>(ExtraNumSynchMeasAvgAux/ExtraExtraNumSynchMeasAvgAux))==0 and CountPRUcurrentTimerValSynchLong!=0){
					...
					if (abs(this->PRUoffsetDriftErrorAvg)<this->PRUoffsetDriftErrorAvgThresh and this->iIterPRUcurrentTimerValSynchLong>NumSynchMeasAvgAux){this->PRUoffsetDriftErrorAvg=0.0;}// Do not apply relative frequency difference if it is below a certain value
...
int GPIO::SendTriggerSignals(int QuadEmitDetecSelecAux, double SynchTrigPeriodAux,unsigned int NumberRepetitionsSignalAux,double* FineSynchAdjValAux,unsigned long long int QPLAFutureTimePointNumber, bool QPLAFlagTestSynchAux){ // Uses output pins to clock subsystems physically generating qubits or entangled qubits
	...
	if (SynchCorrectionTimeFreqNoneFlag==1 or SynchCorrectionTimeFreqNoneFlag==3){ // It has to be a much finer control at PRU level (and also this level) in order to be able to compensate for the very low hardware wandering/drift.
		// Compute the number of quarter passes 8since it is histogram analysis of 4 symbols to add/substract 1 PRU unit of compensation
