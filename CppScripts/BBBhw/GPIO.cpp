	...
	while(true){		
		...
		if (this->ManualSemaphoreExtra==false){// It was possible to execute when needed			
			...
			if (this->ResetPeriodicallyTimerPRU1){
				...
				if (this->iIterPRUcurrentTimerValSynch<static_cast<long long int>(NumSynchMeasAvgAux) and abs(duration_FinalInitialMeasTrig)>ApproxInterruptTime){// Initially run many times so that interrupt handling warms up
				//	auto duration_since_epochTimeNow=(Clock::now()).time_since_epoch();
				//	this->PRUoffsetDriftError=static_cast<double>(fmodl(static_cast<long double>((static_cast<unsigned long long int>(std::chrono::duration_cast<std::chrono::nanoseconds>(duration_since_epochTimeNow).count())/static_cast<unsigned long long int>(TimePRU1synchPeriod)+1)*static_cast<unsigned long long int>(TimePRU1synchPeriod)+static_cast<unsigned long long int>(duration_FinalInitialCountAuxArrayAvg))/static_cast<long double>(PRUclockStepPeriodNanoseconds),static_cast<long double>(iepPRUtimerRange32bits)));
				...
				if ((this->iIterPRUcurrentTimerValSynch<static_cast<long long int>(NumSynchMeasAvgAux) and abs(duration_FinalInitialMeasTrig)>ApproxInterruptTime) or this->iIterPRUcurrentTimerValSynch<static_cast<long long int>(NumSynchMeasAvgAux/2)){// Initially compute the time for interrupt handling
					...
					if (this->iIterPRUcurrentTimerValSynch<static_cast<long long int>(NumSynchMeasAvgAux) and abs(duration_FinalInitialMeasTrig)>ApproxInterruptTime){
						...
						duration_FinalInitialMeasTrig=static_cast<int>(this->PRUcurrentTimerVal-this->PRUcurrentTimerValOldWrap)-static_cast<int>(static_cast<double>(this->iIterPRUcurrentTimerValPass*this->TimePRU1synchPeriod)/static_cast<double>(PRUclockStepPeriodNanoseconds));
					}
...
/////////////
	...
	if (abs(PRUoffsetDriftErrorAbsAvg)>PRUoffsetDriftErrorAbsAvgMax){
		cout << "GPIO::PRUoffsetDriftErrorAbsAvg magnitude is too large: " << PRUoffsetDriftErrorAbsAvg << "Increase PRUoffsetDriftErrorAbsAvgMax." << endl;
	}
	if (this->PRUoffsetDriftErrorAbsAvg<0.0){
		PRUoffsetDriftErrorAbsAvgAux=PRUoffsetDriftErrorAbsAvgMax-fmod(-this->PRUoffsetDriftErrorAbsAvg,PRUoffsetDriftErrorAbsAvgMax);
	}
	else{
		PRUoffsetDriftErrorAbsAvgAux=PRUoffsetDriftErrorAbsAvgMax+fmod(this->PRUoffsetDriftErrorAbsAvg,PRUoffsetDriftErrorAbsAvgMax);
	}
	...
	if (duration_FinalInitialMeasTrigAuxAvg>ApproxInterruptTime){
...
int GPIO::SendTriggerSignals(int QuadEmitDetecSelecAux, double SynchTrigPeriodAux,unsigned int NumberRepetitionsSignalAux,double* FineSynchAdjValAux,unsigned long long int QPLAFutureTimePointNumber){ // Uses output pins to clock subsystems physically generating qubits or entangled qubits
	...
	// The Absolute error is introduced at each signal trigger and timetagging sequence
	if (abs(PRUoffsetDriftErrorAbsAvg)>PRUoffsetDriftErrorAbsAvgMax){
		cout << "GPIO::PRUoffsetDriftErrorAbsAvg magnitude is to large: " << PRUoffsetDriftErrorAbsAvg << "Increase PRUoffsetDriftErrorAbsAvgMax." << endl;
	}
	if (this->PRUoffsetDriftErrorAbsAvg<0.0){
		PRUoffsetDriftErrorAbsAvgAux=PRUoffsetDriftErrorAbsAvgMax-fmod(-this->PRUoffsetDriftErrorAbsAvg,PRUoffsetDriftErrorAbsAvgMax);
	}
