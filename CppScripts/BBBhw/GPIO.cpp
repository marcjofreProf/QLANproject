...
int GPIO::PRUsignalTimerSynchJitterLessInterrupt(){
	...
	while(true){		
		...
		if (this->ManualSemaphoreExtra==false and Clock::now()<(this->TimePointClockCurrentSynchPRU1future-std::chrono::nanoseconds(2*TimePRUcommandDelay))){// It was possible to execute when needed, and still on time to be executed (otherwise skip it to not produce accumulations)			
