		...
		if (Clock::now()<=(this->TimePointClockCurrentSynchPRU1future-std::chrono::nanoseconds(this->TimeClockMargingExtra))){// It was possible to execute when needed
			...
			if (clock_nanosleep(CLOCK_TAI,TIMER_ABSTIME,&requestWhileWait,NULL)==0 and this->ManualSemaphore==false){// Synch barrier. CLOCK_TAI (with steady_clock) instead of CLOCK_REALTIME (with system_clock).
				...
				if (abs(this->PRUoffsetDriftError)<1e7 or this->iIterPRUcurrentTimerValSynch<20){// Do computations
					...
					this->EstimateSynch=(static_cast<double>(this->iIterPRUcurrentTimerValPass*this->TimePRU1synchPeriod)/static_cast<double>(PRUclockStepPeriodNanoseconds))/(static_cast<double>(this->PRUcurrentTimerVal+1*abs(this->PRUoffsetDriftErrorAppliedRaw))-static_cast<double>(this->PRUcurrentTimerValOld-1*abs(this->PRUoffsetDriftErrorAppliedOldRaw)));// Only correct for PRUcurrentTimerValOld with the PRUoffsetDriftErrorAppliedOldRaw to be able to measure the real synch drift and measure it (not affected by the correctoin).
					...
					this->EstimateSynchDirection=(static_cast<double>(this->PRUcurrentTimerVal+0*abs(this->PRUoffsetDriftErrorAppliedRaw)))-(static_cast<double>(this->PRUcurrentTimerValOld-0*abs(this->PRUoffsetDriftErrorAppliedOldRaw))+(static_cast<double>(this->iIterPRUcurrentTimerValPass*this->TimePRU1synchPeriod)/static_cast<double>(PRUclockStepPeriodNanoseconds)));
					...
					// Error averaging
					this->PRUoffsetDriftErrorArray[iIterPRUcurrentTimerValSynch%NumSynchMeasAvgAux]=static_cast<double>(this->PRUoffsetDriftError);
