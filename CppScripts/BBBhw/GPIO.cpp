...
GPIO::GPIO(){// Redeclaration of constructor GPIO when no argument is specified
	...
	// Timetagging
	pru0dataMem_int[DATARAMoffset]=(unsigned int)0; // set to zero means no command. PRU0 idle
	    // Execute program
	...
	// Generate signals
	pru1dataMem_int[DATARAMoffset]=(unsigned int)0; // set to zero means no command. PRU1 idle
...
pru0dataMem_int[DATARAMoffset]=(unsigned int)2; // set to 2 means perform capture
...
if (TimeNow_time_as_count>TimePointFuture_time_as_count){CheckTimeFlag=true;}
	...
	if (pru0dataMem_int[DATARAMoffset] == (unsigned int)1 and CheckTimeFlag==false)// Seems that it checks if it has finished the acquisition
	{
		// we have received the ack!
		this->DDRdumpdata(); // Store to file
		pru0dataMem_int[DATARAMoffset] = (unsigned int)0; // Here clears the value
		fin=true;
		//printf("Ack\n");
	}
...
bool CheckTimeFlag=false;
pru1dataMem_int[DATARAMoffset]=(unsigned int)2; // set to 2 means perform signals
...
do // This is blocking
{
	...
	if (pru1dataMem_int[DATARAMoffset] == (unsigned int)1 and CheckTimeFlag==false)// Seems that it checks if it has finished the sequence
	{	
		pru1dataMem_int[DATARAMoffset] = (unsigned int)0; // Here clears the value
