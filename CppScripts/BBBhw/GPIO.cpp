...
int GPIO::PRUsignalTimerSynchJitterLessInterrupt(){
	...
	while(true){
		...
		// Information
		if (this->ResetPeriodicallyTimerPRU1 and (this->iIterPRUcurrentTimerVal%(1024*NumSynchMeasAvgAux)==0) and this->iIterPRUcurrentTimerValSynchLong>NumSynchMeasAvgAux){
...
unsigned short GPIO::packBits(unsigned short value) {
    ...
    //unsigned short byte0aux = ((value & 0x0010) >> 2) | ((value & 0x0002) << 2) | ((value & 0x0040) >> 5) | ((value & 0x0020) >> 5); // Are the bits 0x0072, moved to 0x000F
