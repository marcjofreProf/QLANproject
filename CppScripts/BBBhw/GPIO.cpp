	...
	while(true){		
		...
		if (this->ManualSemaphoreExtra==false){// It was possible to execute when needed			
			...
			if (this->ResetPeriodicallyTimerPRU1){
				...
				if ((this->iIterPRUcurrentTimerValSynch<static_cast<long long int>(NumSynchMeasAvgAux) and abs(duration_FinalInitialMeasTrig)>(ApproxInterruptTime/2)) or this->iIterPRUcurrentTimerValSynch<static_cast<long long int>(NumSynchMeasAvgAux/2)){// Initially run many times so that interrupt handling warms up
				//	auto duration_since_epochTimeNow=(Clock::now()).time_since_epoch();
				//	this->PRUoffsetDriftError=static_cast<double>(fmodl(static_cast<long double>((static_cast<unsigned long long int>(std::chrono::duration_cast<std::chrono::nanoseconds>(duration_since_epochTimeNow).count())/static_cast<unsigned long long int>(TimePRU1synchPeriod)+1)*static_cast<unsigned long long int>(TimePRU1synchPeriod)+static_cast<unsigned long long int>(duration_FinalInitialCountAuxArrayAvg))/static_cast<long double>(PRUclockStepPeriodNanoseconds),static_cast<long double>(iepPRUtimerRange32bits)));
...
/////////////
	...
	double dPRUoffsetDriftErrorAvg=PRUoffsetDriftErrorAvg*PRUoffsetDriftErrorAbsAvgMax*1000000000.0;//*static_cast<long double>(MultFactorEffSynchPeriod*SynchTrigPeriod)*fmodl(ldTimePointClockTagPRUinitial,static_cast<long double>(MultFactorEffSynchPeriod*SynchTrigPeriod)));
	...
	InstantCorr=static_cast<long long int>(static_cast<long double>((1.0/6.0)*AccumulatedErrorDrift)*static_cast<long double>(MultFactorEffSynchPeriod*SynchTrigPeriod)*fmodl(ldTimePointClockTagPRUinitial,static_cast<long double>(MultFactorEffSynchPeriod*SynchTrigPeriod)));
	...
	InstantCorr=SignAuxInstantCorr*(abs(InstantCorr)%static_cast<long long int>(MultFactorEffSynchPeriod*SynchTrigPeriod));
	ldTimePointClockTagPRUinitial=ldTimePointClockTagPRUinitial+static_cast<long double>(static_cast<long long int>(MultFactorEffSynchPeriod*SynchTrigPeriod)+InstantCorr);// Update the value with the rel. freq. adj.
	pru0dataMem_int[3]=static_cast<unsigned int>(static_cast<long long int>(MultFactorEffSynchPeriod*SynchTrigPeriod)+InstantCorr);// Referenced to the synch trig period
...
int GPIO::SendTriggerSignals(int QuadEmitDetecSelecAux, double SynchTrigPeriodAux,unsigned int NumberRepetitionsSignalAux,double* FineSynchAdjValAux,unsigned long long int QPLAFutureTimePointNumber){ // Uses output pins to clock subsystems physically generating qubits or entangled qubits
	...
	InstantCorr=SignAuxInstantCorr*(abs(InstantCorr)%static_cast<long long int>(MultFactorEffSynchPeriod*SynchTrigPeriod));
	ldTimePointClockTagPRUinitial=ldTimePointClockTagPRUinitial+static_cast<long double>(static_cast<long long int>(MultFactorEffSynchPeriod*SynchTrigPeriod)+InstantCorr);// Update the value with the rel. freq. adj.
	pru1dataMem_int[2]=static_cast<unsigned int>(static_cast<long long int>(MultFactorEffSynchPeriod*SynchTrigPeriod)+InstantCorr);// Referenced to the synch trig period
