			...
			if (this->ResetPeriodicallyTimerPRU1){
				...
				this->ManualSemaphoreExtra=true;
					...
					if (this->iIterPRUcurrentTimerVal!=0){
						this->PRUoffsetDriftErrorArray[iIterPRUcurrentTimerValSynch%ExtraNumSynchMeasAvgAux]=fmodl(this->PRUoffsetDriftError-(1.0-EstimateSynchAvg)*static_cast<double>(this->iIterPRUcurrentTimerVal*this->TimePRU1synchPeriod)/static_cast<double>(PRUclockStepPeriodNanoseconds),static_cast<double>(iepPRUtimerRange32bits));	
					}
					else{
...
long double InstantErrorCorr=-((1.0-this->AdjPulseSynchCoeffAverage)*(static_cast<long double>(std::chrono::duration_cast<std::chrono::nanoseconds>(TimePointClockTagPRUinitial-TimePointClockSendCommandFinal).count())/static_cast<long double>(PRUclockStepPeriodNanoseconds))+static_cast<long double>(AccumulatedErrorDrift));
long double SignAux;
...
long double InstantErrorCorr=-((1.0-this->AdjPulseSynchCoeffAverage)*(static_cast<long double>(std::chrono::duration_cast<std::chrono::nanoseconds>(TimePointFutureSynch-TimePointClockSendCommandFinal).count())/static_cast<long double>(PRUclockStepPeriodNanoseconds))+static_cast<long double>(AccumulatedErrorDrift));
long double SignAux;
if (InstantErrorCorr>0.0){SignAux=1.0;}
else if (InstantErrorCorr<0.0){SignAux=-1.0;}
else {InstantErrorCorr=0.0;}
