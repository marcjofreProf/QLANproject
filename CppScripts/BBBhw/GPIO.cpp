...
pru1dataMem_int[0]=static_cast<unsigned int>(this->NumberRepetitionsSignal); // set the number of repetitions
pru1dataMem_int[3]=static_cast<unsigned int>(0.75*this->SynchTrigPeriod);// Indicate period of the sequence signal, so that it falls correctly and is picked up by the Signal PRU. Link between system clock and PRU clock. It has to be a power of 2
...
double PercentageToEndDurationTag=0.1;// Estimation of where the time is tag produced. The lower the closer to the interrupt exit
