...
int GPIO::PRUsignalTimerSynchJitterLessInterrupt(){
	...
	while(true){
		...
		if (this->ManualSemaphoreExtra==false and clock_nanosleep(CLOCK_REALTIME,TIMER_ABSTIME,&requestWhileWait,NULL)==0){// It was possible to execute when needed, and still on time to be executed (otherwise skip it to not produce accumulations)			
