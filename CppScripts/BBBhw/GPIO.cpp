		...
		if (this->ManualSemaphoreExtra==false){// It was possible to execute when needed			
			...
			if (this->ResetPeriodicallyTimerPRU1){
				...
				this->ManualSemaphoreExtra=true;
					...
					this->AccumulatedErrorDrift=DoubleMedianFilterSubArray(PRUoffsetDriftErrorArray,NumSynchMeasAvgAux);					
	...
	while(true){
		...
		if (this->ManualSemaphoreExtra==false){// It was possible to execute when needed			
			...
			if (this->ResetPeriodicallyTimerPRU1){
				...
				// Below for synch calculation compensation
				duration_FinalInitialCountAux=static_cast<double>(duration_FinalInitialMeasTrig);				
				duration_FinalInitialCountAuxArray[iIterPRUcurrentTimerValSynch%ExtraNumSynchMeasAvgAux]=this->duration_FinalInitialCountAux;
				duration_FinalInitialCountAuxArrayAvg=DoubleMedianFilterSubArray(duration_FinalInitialCountAuxArray,ExtraNumSynchMeasAvgAux);					
...
int SynchRem=static_cast<int>((static_cast<long double>(2.5*SynchTrigPeriod)-fmodl((static_cast<long double>(std::chrono::duration_cast<std::chrono::nanoseconds>(TimePointClockTagPRUinitial.time_since_epoch()).count())/static_cast<long double>(PRUclockStepPeriodNanoseconds)),static_cast<long double>(SynchTrigPeriod)))*static_cast<long double>(PRUclockStepPeriodNanoseconds));// For time stamping it waits 1.5 +1, being the +1 because in the signal trigger it is added a whole period to compensate for clock drift (instead of acting directly on IEP counter which it is not a good idea)
...
pru1dataMem_int[2]=static_cast<unsigned int>(FineSynchAdjOffVal)+0*static_cast<unsigned int>((static_cast<unsigned long long int>((static_cast<long double>(SynchTrigPeriod)+static_cast<long double>(this->AccumulatedErrorDrift))*(static_cast<long double>(std::chrono::duration_cast<std::chrono::nanoseconds>(TimePointFutureSynch.time_since_epoch()).count())/static_cast<long double>(TimePRU1synchPeriod))))%static_cast<unsigned long long int>(2*SynchTrigPeriod))+static_cast<unsigned int>((static_cast<unsigned long long int>(static_cast<long double>(FineSynchAdjFreqVal)*(static_cast<long double>(std::chrono::duration_cast<std::chrono::nanoseconds>(TimePointFutureSynch.time_since_epoch()).count())/static_cast<long double>(TimePRU1synchPeriod))))%static_cast<unsigned long long int>(SynchTrigPeriod));//static_cast<unsigned int>(static_cast<long long int>(SynchTrigPeriod)+static_cast<long long int>(FineSynchAdjOffVal)+(static_cast<unsigned long long int>(FineSynchAdjFreqVal)*this->iIterPRUcurrentTimerVal)%static_cast<unsigned long long int>(SynchTrigPeriod));// Regular offset of trig signal//static_cast<unsigned int>(// Use it to indicate some offset for time. It is dependent for each node and channel. With respect SynchTrigPeriod sp that it can be extra or less. It has to be changing to adjust the variation as a frequency offset
