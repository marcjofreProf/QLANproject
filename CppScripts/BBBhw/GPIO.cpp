	...
	if (streamSynchpru.is_open()){
		...
		if (NumSynchPulsesRed>3){//At least four points (two cycles separated by one cycle in between), we can generate a calibration curve
			...
			for (int iIter=0;iIter<(NumSynchPulsesRed-3);iIter++){
				...
				CoeffSynchAdjAux3=(double)((unsigned long long int)(((double)(SynchPulsesTags[iIter+1]-SynchPulsesTags[iIter+0])+PeriodCountsPulseAdj/2.0)/PeriodCountsPulseAdj))*1.0; // Distill how many pulse synch periods passes...1, 2, 3....To round ot the nearest integer value add half of the dividend to the divisor
				...
				if (CoeffSynchAdjAux1!=0.0 and CoeffSynchAdjAux3!=0.0 and CoeffSynchAdjAux4!=0.0){
					AdjPulseSynchCoeff=AdjPulseSynchCoeff+(CoeffSynchAdjAux2/(double)(CoeffSynchAdjAux0));//+(CoeffSynchAdjAux2/CoeffSynchAdjAux1);
					NumAvgAux++;
				}
			}
