...
unsigned short GPIO::packBits(unsigned short value) {
    ...
    unsigned short byte0aux =  ((value & 0x0040) >> 5) | ((value & 0x0020) >> 3) | ((value & 0x0010) >> 1) | ((value & 0x0002) >> 1); // Are the bits 0x0072, moved to 0x000F
    unsigned short byte1aux = (value & 0x0080) | ((value & 0x0004) << 4) | ((value & 0x0008) << 2) | ((value & 0x0001)<<4); // Are the bits 0x008D, moved to 0x00F0
    unsigned short byte2aux = ((value & 0xF000) >> 4); // To be check that the ordering is correct!!!! // Byte 1 shifts to the right four bit positions (the interesting ones) // Are the bits 0xF000, moved to 0x0F00
