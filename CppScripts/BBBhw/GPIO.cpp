...
bool GPIO::setMaxRrPriority(){// For rapidly handling interrupts
	...
	int Nice_priority=77;// Higher priority. Very important parameter to have stability of the measurements. Slightly larger than the priorities for clock control (ptp4l,...) but larger than for the general program
...
int GPIO::PRUsignalTimerSynchJitterLessInterrupt(){
	...
	while(true){		
		...
		// Information
		if (this->ResetPeriodicallyTimerPRU1 and (this->iIterPRUcurrentTimerVal%(2*NumSynchMeasAvgAux)==0) and this->iIterPRUcurrentTimerValSynchLong>NumSynchMeasAvgAux){//if (this->ResetPeriodicallyTimerPRU1 and (this->iIterPRUcurrentTimerVal%(512*NumSynchMeasAvgAux)==0) and this->iIterPRUcurrentTimerValSynchLong>NumSynchMeasAvgAux){
