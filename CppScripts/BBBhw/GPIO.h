...
private:// Variables
	...
	double AccumulatedErrorDriftThresh=5e-8; //Threshold value to not apply relative frequency difference
	...
	double SigONPeriod=10.0; // ON time (duty cycle) of the signal, in PRU time. It cannot be smaller than 6. It has to be multiple of 2. Somehow, it cannot be very large because the PRUs stall (maybe a voltage issue or device tree overlay...)
	...
	unsigned int TTGcoincWin=50;// It cannot be smaller than 2. Timetagging coincidence window length In PRU units. It reduces time resolution of the detected qubits.
