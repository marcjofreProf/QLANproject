...
private:// Variables
	...
	int NumSynchMeasAvgAux=101;//51; // Num averages to compute the relative frequency difference. Better to be odd number.
	int ExtraNumSynchMeasAvgAux=91; // Averaging for computing current absolute time offset
	...
	unsigned long long int DistTimePRU1synchPeriod=1; // Number of passes with respect TimePRU1synchPeriod, in order to compute both the absolute time difference and the relative frequency difference
