...
#define NumSynchMeasAvgAux 	101 //351; // Num averages to compute the relative frequency difference. Better to be odd number.
#define ExtraNumSynchMeasAvgAux 	101 // 351 // Averaging for computing current absolute time offset
...
private:// Variables
	...
	int ApproxInterruptTime=250000; // Typical time of interrupt time duration is 5000 with simple busy wait; around 8000 with busy wait with yield; 100000 with sleep_for()
	...
	double SigONPeriod=6.0; // ON time (duty cycle) of the signal, in PRU time. It cannot be smaller than 6. It has to be multiple of 2. Somehow, it cannot be very large because the PRUs stall (maybe a voltage issue or device tree overlay...)
