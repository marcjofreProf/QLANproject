...
private:// Variables
	...
	int WaitTimeInterruptPRU0=2000000; // In microseconds. Two seconds because the lock can go up to 5 seconds, but has an active range of 2.5 seconds, then discounting some clock misadjustments. 2 s is safe.
	...
	//bool finPRU0;
	// PRU Signal
	unsigned int NumberRepetitionsSignal=131072;// Sets the equivalent MTU (Maximum Transmission Unit) for quantum (together with the clock time) - it could be named Quantum MTU
	int retInterruptsPRU1;
	int WaitTimeInterruptPRU1=10000000; // In microseconds
