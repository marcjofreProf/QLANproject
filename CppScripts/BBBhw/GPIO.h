...
#define NumSynchMeasAvgAux 	191 //191; // Num averages to compute the relative frequency difference. Better to be odd number.
#define ExtraNumSynchMeasAvgAux 	191 // 191 // Averaging for computing current absolute time offset
...
#define ExtraExtraNumSynchMeasAvgAux 	161 // 131 Averaging for computing current relative frequency diference.
...
private:// Variables
	...
	int PriorityValTop=75; // Top priority for critical operations
	// Semaphore
	...
	double truncatedSynchTrigPeriodPeriodic=128.0;// Multiple of power of 2, for the periodic monitoring to content the relative frequency difference. Gives stability to the calculation of the relative frequency difference.
	...
	double SigONPeriod=16.0; // Half the time jitter in PRU units // ON time (duty cycle) of the signal, in PRU time. It cannot be smaller than 6. It has to be multiple of 2. Somehow, it cannot be very large because the PRUs stall (maybe a voltage issue or device tree overlay...)
	...
	unsigned int TTGcoincWin=10; // Less than half of the signal time on the time jitter in PRU units. 1;// It cannot be smaller than 1. Timetagging coincidence window length In PRU units. It reduces time resolution of the detected qubits.
