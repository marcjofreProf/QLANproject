...
private:// Variables
	...
	int ApproxInterruptTime=8000; // Typical time of interrupt time duration is 5000 with simple busy wait; around 8000 with busy wait with yield
	int NumSynchMeasAvgAux=5;//51; // Num averages to compute the relative frequency difference. Better to be odd number.
	int ExtraNumSynchMeasAvgAux=5; // Averaging for computing current absolute time offset
