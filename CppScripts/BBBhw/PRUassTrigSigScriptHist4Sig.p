...
#define DELAYMODULE	32768// Slightly above the std jitter of the 24 MHz hardware clock//65536 // Not used. The whole histogram period. It has to be divisible by the 2^32 total clock register.
#define DELAYHALFMODULE	32767//65535 // Not used. One unit less than the power of two required of the period of the histogram which is 65535-1=2^16-1.
#define HALFMODULE	16384 // To center the arrival of the interrupt. It is half of DELAYMODULE
...
PSEUDOSYNCH:// Only needed at the beggining to remove the unsynchronisms of starting to emit at specific bins for the histogram or signal. It is not meant to correct the absolute time, but to correct for the difference in time of emission due to entering thorugh an interrupt. So the period should be small (not 65536). For instance (power of 2) larger than the below calculations and slightly larger than the interrupt time (maybe 40 60 counts). Maybe 64 is a good number.
	...
	SUB	r0, r0, r8 // Center the arrival of the interrupt
